...
class Experiment:
    ...
    def __init__(self, max_sim:int, load_previous:bool,
            fork:int, levels:int, k:int, algorithm:str,
            dist:str, nosave:bool, dist_parameters:dict,
            c:float, fixed_size:bool, lgllr:bool,
            processes:int, verbose:bool,
            datafolder:str, index:int, max_samples:int,
        ...
        with self.add_attribute_names_to_list(self.words):
            ...
            if self.hierarchical:
                self.local_test = 'fixed' if fixed_size else 'active'
            if algorithm in ['HDS','DS']:
        ...
        if self.hierarchical:
            seq_args['fixed_size'] = fixed_size
        if self.algorithm in ['DS', 'HDS']:
